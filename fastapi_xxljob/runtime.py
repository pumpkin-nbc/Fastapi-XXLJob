"""
FastAPI-XXLJob 运行时对象。

FastAPI-XXLJob runtime object.
...
class XXLJobRuntime:
    """
    单个 FastAPI 应用的 FastAPI-XXLJob 运行时，保存在
    ``app.state.xxljob``。

    The per-application FastAPI-XXLJob runtime, stored in
    ``app.state.xxljob``.
    """
    ...
    def close(self) -> None:
        ...
        logger.info("FastAPI-XXLJob runtime shutdown started.")
        try:
            ...
            logger.exception("FastAPI-XXLJob runtime shutdown failed.")
