"""
保存 FastAPI 项目注册的 XXL-JOB 请求处理函数。

Stores the XXL-JOB request callbacks registered by the FastAPI project.
...
CallbackResult = Union[object, Awaitable[object]]
RunCallback = Callable[[TriggerRequest], CallbackResult]
IdleBeatCallback = Callable[[IdleBeatRequest], CallbackResult]
KillCallback = Callable[[KillRequest], CallbackResult]
LogCallback = Callable[[LogRequest], Union[LogResponse, Awaitable[object], object]]
...
class CallbackRegistry:
    """
    保存单个 FastAPI 应用注册的 XXL-JOB 请求处理函数。

    每个 FastAPI 应用拥有独立的注册表实例，从而实现应用间隔离。

    Holds the named Run callbacks plus the three other request-callbacks for a
    single FastAPI application. Each application owns an independent registry.
    """
