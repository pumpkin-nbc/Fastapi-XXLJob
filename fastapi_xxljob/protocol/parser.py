...
async def read_limited_request_body(
    request: "Request", max_request_size: int
) -> bytes:
    """Read an ASGI request body without buffering beyond the configured limit."""
    raw_length = request.headers.get("content-length")
    if raw_length is not None:
        try:
            content_length = int(raw_length)
        except ValueError as exc:
            raise RequestParseError("invalid Content-Length header") from exc
        if content_length < 0:
            raise RequestParseError("invalid Content-Length header")
