"""
FastAPI-XXLJob 插件运行状态模型。

FastAPI-XXLJob plugin runtime-status model.

该模型只描述 FastAPI-XXLJob 插件自身的状态（是否启用、是否自动注册、最近一次注册
结果等），绝不描述业务任务状态，也不包含 Access Token。

This model only describes the state of the FastAPI-XXLJob plugin itself (whether
...
class XXLJobStatus:
    """
    FastAPI-XXLJob 插件的只读运行状态快照。

    A read-only snapshot of the FastAPI-XXLJob plugin runtime status.
    """
