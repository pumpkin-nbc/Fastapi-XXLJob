"""FastAPI-XXLJob configuration loading and validation."""
...
DEFAULTS: dict = {
    ...
    "XXL_JOB_LOG_FILENAME": "fastapi-xxljob.log",
...
class XXLJobConfig:
    """
    经过校验的 FastAPI-XXLJob 运行时配置。

    Validated FastAPI-XXLJob runtime configuration.
    """

    enabled: bool = True
    admin_addresses: List[str] = field(default_factory=list)
    access_token: str = ""
    executor_app_name: str = "fastapi-xxljob-executor"
    ...
    log_path: str = "./logs"
    log_filename: str = "fastapi-xxljob.log"
    ...
    def from_mapping(cls, config: Mapping[str, Any]) -> "XXLJobConfig":
        ...
        Build and validate the configuration from an explicit mapping.
        """
        ...
        if enabled:
            ...
            # URL/ASGI path，也不会组合出一个实际不会使用的执行器地址。
            # disabled is the master switch: retain the typed strings without
            # interpreting them as URLs/ASGI paths or composing an unused URL.
...
def _normalize_prefix(prefix: str) -> str:
    ...
    if any(
        ...
        raise XXLJobConfigError(
            "XXL_JOB_ROUTE_PREFIX must be a static ASGI path without "
...
def _validate_log_format(log_format: str, date_format: str) -> None:
    try:
        ...
        record = logging.LogRecord(
            name="fastapi_xxljob.validation",
            level=logging.INFO,
            pathname=__file__,
            lineno=1,
            msg="validation",
