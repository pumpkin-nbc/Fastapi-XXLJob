"""Per-application logging management for FastAPI-XXLJob."""
...
class XXLJobLogManager:
    """Own the managed handlers for one FastAPI application runtime."""

    def __init__(self, app: "FastAPI", config: XXLJobConfig) -> None:
        runtime_id = _next_runtime_id()
        app_name = getattr(app, "title", None) or "app"
        self.name = (
            f"fastapi_xxljob.app.{_safe_app_name(app_name)}.{runtime_id}"
        )
    ...
    def _configure(self) -> None:
        ...
        try:
            ...
            raise XXLJobInitializationError(
                "Failed to initialize FastAPI-XXLJob logging "
                f"({type(exc).__name__})."
            ) from exc
    ...
    def _prepare_handler(
        ...
        handler._fastapi_xxljob_managed = True  # type: ignore[attr-defined]
        handler._fastapi_xxljob_handler_type = handler_type  # type: ignore[attr-defined]
        handler._fastapi_xxljob_owner = self.name  # type: ignore[attr-defined]
