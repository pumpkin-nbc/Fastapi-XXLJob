...
from fastapi_xxljob.protocol.parser import RequestParseError, _read_limited_body
from fastapi_xxljob.protocol.validator import check_access_token
...
def test_body_over_limit_returns_json():
    ...
    with TestClient(app) as client:
        resp = client.post(
            "/run",
            content=b'{"jobId": 123456789012345}',
...
def test_param_over_limit_returns_json():
    from fastapi_xxljob import XXLJobResponse
...
def test_chinese_param_length_counts_characters():
    ...
    with TestClient(app) as client:
        ...
        bad = client.post(
            "/run", json={"jobId": 1, "executorParams": "参数值多"}
        )
