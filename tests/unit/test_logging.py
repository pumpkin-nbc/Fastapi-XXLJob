...
from fastapi_xxljob.client import CallResult
from fastapi_xxljob.exceptions import XXLJobConfigError, XXLJobInitializationError
from tests.conftest import BASE_CONFIG, make_app


def _runtime(app):
    return app.state.xxljob
...
def test_logging_defaults_are_disabled_without_side_effects(tmp_path, capsys):
    ...
    app = FastAPI(title="default-logging")
    log_dir = tmp_path / "must-not-exist"
    config = dict(BASE_CONFIG)
    config["XXL_JOB_LOG_PATH"] = str(log_dir)

    FastAPIXXLJob(app, config)
    _log(app)
...
def test_file_logging_uses_absolute_path_and_no_console(tmp_path, capsys):
    ...
    assert "INFO|fastapi_xxljob.app." in content
    assert capsys.readouterr() == ("", "")
...
def test_no_managed_targets_preserves_host_logger_configuration(tmp_path):
    package_logger = logging.getLogger("fastapi_xxljob")
...
def test_protocol_logs_safe_failure_categories(tmp_path, capsys):
    ...
    with TestClient(app) as client:
        ...
        client.post(
            "/run",
            content=b"{not-json-business-secret",
            headers={
...
def test_invalid_logging_configuration_rolls_back(key, value, tmp_path):
    ...
    assert getattr(app.state, "xxljob", None) is None
    assert not (tmp_path / "must-not-exist").exists()
...
def test_logging_initialization_failure_has_no_runtime_state(tmp_path):
    ...
    app = FastAPI(title="logging-init-failure")
    config = dict(BASE_CONFIG)
    config.update(
        XXL_JOB_LOG_ENABLED=True,
...
def test_callback_event_does_not_log_handle_message(tmp_path, capsys, mocker):
    ...
    mocker.patch("fastapi_xxljob.client.requests.post", return_value=response)

    result = ext.callback_success_sync(1, 2, "handle-message-secret", app=app)
...
def test_admin_failover_registry_renewal_removal_and_callback_events(
    ...
    assert ext.register_executor_sync(app).success is True
    assert runtime.registry_service.register_once_result(
        operation="renewal"
    ).success is True
    assert ext.callback_success_sync(1, 2, "handle-message-secret", app=app).success is False
    assert ext.remove_executor_sync(app).success is True
