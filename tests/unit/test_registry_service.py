...
from fastapi_xxljob.exceptions import XXLJobConfigError
from fastapi_xxljob.registry.registry_service import (
    RegistryService,
...
def test_disabled_sync_api_updates_only_local_safe_snapshot(
    ...
    assert result == CallResult(
        success=False,
        error="FastAPI-XXLJob is disabled.",
        error_type="config",
        attempt_count=0,
    )
...
def test_enabled_remote_entrypoints_validate_before_side_effects(method, mocker):
    ...
    service = RegistryService(config, admin)
    thread = mocker.patch(
        "fastapi_xxljob.registry.registry_service.threading.Thread"
    )
...
def test_fork_status_and_running_replace_state_without_parent_lock(mocker):
    ...
    mocker.patch(
        "fastapi_xxljob.registry.registry_service.os.getpid",
...
def test_fork_disabled_snapshot_uses_blank_child_state(
    ...
    child = service._process_state
    assert child is not parent
    assert result.error == "FastAPI-XXLJob is disabled."
