...
from fastapi_xxljob import CallbackRequest
from fastapi_xxljob.client.callback_client import CALLBACK_PATH, CallbackClient
from fastapi_xxljob.config import XXLJobConfig
from fastapi_xxljob.exceptions import XXLJobValidationError
...
def test_callback_many_bad_type_raises(mocker):
    mocker.patch("fastapi_xxljob.client.requests.post")
    client = CallbackClient(make_config())
    with pytest.raises(XXLJobValidationError):
