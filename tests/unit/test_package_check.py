...
METADATA = "\n".join(
    (
        ...
        "https://github.com/pumpkin-nbc/Fastapi-XXLJob/tree/master/docs",
        "Project-URL: Source, https://github.com/pumpkin-nbc/Fastapi-XXLJob",
        "Project-URL: Changelog, "
        "https://github.com/pumpkin-nbc/Fastapi-XXLJob/blob/master/CHANGELOG.md",
        "",
    )
)
...
def test_valid_wheel_record_and_directory_entry(tmp_path):
    path = _write_wheel(tmp_path, directories=("fastapi_xxljob/data",))
...
def test_duplicate_zip_file_path_is_rejected(tmp_path):
    path = _write_wheel(
        tmp_path, duplicate_file="fastapi_xxljob/__init__.py"
    )
...
def test_unrecorded_file_and_missing_file_record_are_rejected(tmp_path):
    files = _wheel_files()
    files["fastapi_xxljob/extra.py"] = b"extra\n"
    rows = _record_rows(_wheel_files())
    rows.insert(-1, ["fastapi_xxljob/missing.py", _hash(b"missing"), "7"])
...
def test_forbidden_project_files_are_rejected(tmp_path):
    files = _wheel_files()
    files["fastapi_xxljob/__pycache__/module.pyc"] = b"cache"
